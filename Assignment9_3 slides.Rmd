...
```{r, echo=T,, warning=F, message=F, comment=NA, render=T}
library(AER)
data("HousePrices")
head(HousePrices,3)
```

Goal of this app is to help user find suitable house from dataset with multiple variables. All data points are displayed on price-lotsize plot. App prediction algorithm, based on simple linear model helps in house price estimation. Dot color indicates if house is overestimated (red) or underestimated (blue). There is also ability to display price and actual-predicted price difference on the plot (it's better to use this when just few data points remained). 

Also algorithm produces two prediction lines - for minimum and maximum parameters from selected ranges.
If there is no houses with specified parameters, lines should help to estimate predicted price.

## House Prices App User Interface 

```{r, message=F, warning=F, echo=T, eval=F, class.source="bg-success"}
#This code chunk just demonstrates UI code, so eval=FALSE
library(shiny)
library(shinythemes)
